...
def cnn_model(trainingData, trainingLabels, testData, testLabels):
    ...
    epochs = 20 #Settled on 25. Ultimately there is an optimal number (not too high or too low).
    ...
    model.add(Conv1D(filters=1, kernel_size=5, activation='relu', input_shape=(100, 1)))#each point as 100 values
    model.add(Conv1D(filters=1, kernel_size=5, activation='relu'))#only 1 filter for each as data is pretty arbitrary
...
if __name__ == '__main__':
    ...
    with open(r"C:\Users\bradb\Documents\Data_Generator\synthetic_data.csv", "r") as file:
        buffer = csv.reader(file)
        trainingList = list(buffer)

    with open(r"C:\Users\bradb\Documents\Data_Generator\synthetic_data_labels.csv", "r") as file2:
